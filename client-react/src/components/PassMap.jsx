...
export default function PassMap({ playerId, season }) {
    ...
    axios
      ...
      .then(res => {
        if (res.data.passes) {
          ...
          setError(t('visualization.passMap.errorData'));
        }
      })
      .catch(err => {
        setError(t('visualization.passMap.errorFetch'));
        console.error(err);
      });
  }, [playerId, season]);
  ...
  return (
    <div>
      <h3>{t('visualization.passMap.title')}</h3>
      {error && <p style={{ color: "red" }}>{error}</p>}
      <div style={{ marginBottom: "10px" }}>
        <label><input type="checkbox" checked={filters.completed} onChange={() => handleFilterChange("completed")} /> {t('visualization.passMap.completed')}</label>
        <label style={{ marginLeft: "10px" }}><input type="checkbox" checked={filters.incomplete} onChange={() => handleFilterChange("incomplete")} /> {t('visualization.passMap.incomplete')}</label>
        <label style={{ marginLeft: "10px" }}><input type="checkbox" checked={filters.assists} onChange={() => handleFilterChange("assists")} /> {t('visualization.passMap.assists')}</label>
        <label style={{ marginLeft: "10px" }}><input type="checkbox" checked={filters.finalThird} onChange={() => handleFilterChange("finalThird")} /> {t('visualization.passMap.finalThird')}</label>
      </div>
      <Stage width={canvasWidth} height={canvasHeight}>
        <Layer>
          {drawPitch()}
      ...
      <div style={{ marginTop: "10px" }}>
        <p>{t('visualization.passMap.completionRate')} {stats.completionRate}%</p>
        <p>{t('visualization.passMap.finalThirdRate')} {stats.finalThirdCompletionRate}%</p>
        <p>{t('visualization.passMap.totalAssists')} {stats.totalAssists}</p>
      </div>
      <div style={{ marginTop: "20px" }}>
        <h4>{t('visualization.passMap.zoneCompletion')}</h4>
        {heatmapUrl ? (
          <>
            <img
              key={heatmapUrl}
              src={heatmapUrl}
              alt={t('visualization.passMap.heatmapAlt')}
              style={{ maxWidth: "100%", border: "1px solid #ccc", display: 'block' }}
              onError={(e) => { e.target.style.display = 'none'; if(e.target.nextSibling) e.target.nextSibling.style.display = 'block'; }}
            />
            <p style={{ display: 'none', color: 'red' }}>{t('visualization.passMap.errorLoading')}</p>
          </>
        ) : (
          <p>{t('visualization.passMap.loading')}</p>
        )}
      </div>
    </div>
  );
