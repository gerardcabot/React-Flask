...
# --- Importacions del mòdul d'entrenament ---
# Aquestes funcions són necessàries per a les rutes de predicció i anàlisi de mètriques
from model_trainer.trainer_v2_15_16 import (
...
# --- Configuració de la Connexió a Cloudflare R2 ---
...
if all([R2_ENDPOINT_URL, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME]):
    ...
    logger.warning("R2 environment variables not fully set. S3 client not initialized. App will likely fail.")

# --- Inicialització de l'Aplicació Flask ---
app = Flask(__name__)
# CORS configurat explícitament per a l'origen del frontend a Vercel
CORS(app, resources={r"/*": {"origins": "https://react-flask-psi.vercel.app"}})

# --- Càrrega de Dades Inicial (de manera resilient) ---
...
if s3_client:
    try:
        ...
        logger.info(f"Successfully loaded {len(player_index_main_data)} players from player_index.json.")
    except Exception as e:
        logger.error(f"CRITICAL STARTUP ERROR: Could not load player_index.json from R2. App will run with an empty player list. Error: {e}", exc_info=True)
else:
    logger.error("CRITICAL STARTUP ERROR: S3 client not initialized. App will run with an empty player list.")
...
def format_base_feature_label(feature_name):
    label = feature_name.replace("_p90_sqrt_", " P90 √").replace("_p90", " P90").replace("_sqrt_", " √")
    label = label.replace("_kpi", " KPI").replace("_inv_kpi_base", " (Inv. Base)")
    label = label.replace("count_", "Count of ").replace("sum_", "Sum of ").replace("_", " ")
...
def load_player_data(player_id, season):
    if not s3_client:
        logger.error("Cannot load player data from R2: S3 client not initialized.")
        return None
    ...
    try:
        ...
        df = pd.read_csv(StringIO(response['Body'].read().decode('utf-8')), low_memory=False)
        
        loc_cols = [col for col in df.columns if 'location' in col or 'end_location' in col]
        for col in loc_cols:
            if col in df.columns: df[col] = df[col].apply(safe_literal_eval)
        
        bool_cols = sorted(list(set(['counterpress', 'offensive', 'recovery_failure', 'deflection', 'save_block', 'aerial_won', 'nutmeg', 'overrun', 'no_touch', 'advantage', 'penalty', 'defensive', 'backheel', 'miscommunication', 'cross', 'cut_back', 'switch', 'shot_assist', 'goal_assist', 'follows_dribble', 'first_time', 'open_goal', 'under_pressure', 'out'])))
        ...
        numeric_cols = ['duration', 'pass_length', 'pass_angle', 'shot_statsbomb_xg', 'statsbomb_xg']
...
@app.route("/")
def home():
    return jsonify({"message": "Welcome to the Estrelles del Futur API."})

@app.route("/players")
def players_route():
    if not player_index_main_data:
        return jsonify({"error": "Player list could not be loaded from the source."}), 503
    try:
        player_list = [
        ...
        logger.error(f"Error in /players route: {e}", exc_info=True)
        return jsonify({"error": "An internal error occurred while processing the player list."}), 500
...
def player_events_route():
    ...
    try:
        ...
        if df is None or df.empty:
            return jsonify({"error": "No data found for this player and season"}), 404
        return df.to_json(orient="records", date_format="iso", default_handler=str)
    except Exception as e:
        logger.error(f"Error in /player_events route: {e}", exc_info=True)
...
def pass_completion_heatmap_route():
    player_id = request.args.get("player_id")
    season = request.args.get("season")
    if not all([player_id, season, R2_PUBLIC_URL]):
        return jsonify({"error": "Missing parameters or server configuration for heatmap URL"}), 400
    image_filename = f"{player_id}_{season}_pass_completion_heatmap.png"
    image_url = f"{R2_PUBLIC_URL}/{image_filename}"
...
def shot_map_route():
    ...
    try:
        ...
        df_shots = df[df.get("type") == "Shot"].copy()
        if df_shots.empty or 'location' not in df_shots.columns:
            return jsonify({"shots": []})
        
        df_shots["x"] = df_shots["location"].apply(lambda loc: loc[0] if isinstance(loc, (list, tuple)) and len(loc) > 0 else None)
        df_shots["y"] = df_shots["location"].apply(lambda loc: loc[1] if isinstance(loc, (list, tuple)) and len(loc) > 1 else None)
        df_shots["is_goal"] = df_shots["shot_outcome_name"].astype(str) == "Goal"
        df_shots.dropna(subset=['x', 'y'], inplace=True)
        ...
        logger.error(f"Error in /shot_map route: {e}", exc_info=True)
...
def available_aggregated_metrics_route():
    try:
        ...
        formatted_metrics = [
            ...
            if not any(name.startswith(p) for p in ["player_", "target_season_", "general_position_", "matches_played_"]) and name != "season_numeric"
        ]
        return jsonify({"available_metrics": sorted(formatted_metrics, key=lambda x: x["label"])})
    except Exception as e:
...
def player_seasonal_metric_trend_route():
    ...
    try:
        ...
        if not player_metadata or not player_metadata.get("seasons"):
            return jsonify({"error": "Player not found or has no seasons"}), 404

        minutes_df = pd.read_csv(StringIO(s3_client.get_object(Bucket=R2_BUCKET_NAME, Key="data/player_season_minutes_with_names.csv")['Body'].read().decode('utf-8')))
        minutes_df['season_name_std'] = minutes_df['season_name'].str.replace('/', '_', regex=False)
        
        trend_data = []
        for season_str in sorted(player_metadata["seasons"]):
            minutes_row = minutes_df[(minutes_df['player_id'].astype(str) == player_id) & (minutes_df['season_name_std'] == season_str)]
            total_minutes = minutes_row['total_minutes_played'].iloc[0] if not minutes_row.empty else 0.0
            num_90s = trainer_safe_division(total_minutes, 90.0)
            age = get_age_at_fixed_point_in_season(player_metadata["dob"], season_str) or 0
            season_numeric = int(season_str.split('_')[0])
            
            df_events = load_player_data(player_id, season_str)
            if df_events is None: df_events = pd.DataFrame()
            
            base_features = trainer_extract_base_features(df_events, age, season_numeric, num_90s)
            metric_value = base_features.get(metric, 0.0)
            
            trend_data.append({"season": season_str, "value": float(metric_value), "metric_name": metric})
        
        return jsonify({"trend_data": trend_data, "metric_label": format_base_feature_label(metric), "metric_id": metric})
    except Exception as e:
        logger.error(f"Error in /player_seasonal_metric_trend: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
...
def handle_build_custom_model():
    ...
    if not all([github_user, github_repo, github_token]):
        logger.error("GitHub Actions environment variables are not set on the server.")
        return jsonify({"error": "Server is not configured to trigger background jobs."}), 500

    custom_model_id = f"{data.get('model_name', 'custom').replace(' ', '_')}_{uuid.uuid4().hex[:6]}"
    ...
    payload = {
        ...
        "client_payload": {
            ...
            "impact_kpis": data.get("impact_kpis"),
    ...
    try:
        ...
        return jsonify({
            "message": f"Model build for {data.get('position_group')} (ID: {custom_model_id}) has started. It will be available shortly.",
            "custom_model_id": custom_model_id
        }), 202
    except requests.exceptions.RequestException as e:
        logger.error(f"Error dispatching GitHub Action for model '{custom_model_id}': {e}")
        return jsonify({"error": f"Failed to start the background training job. GitHub API responded with: {e.response.text if e.response else 'No response'}"}), 500


@app.route("/scouting_predict")
def scouting_predict_route():
    # Aquesta és una versió completa i corregida de la teva ruta de predicció
    ...
    if not all([player_id_str, season_to_predict_for, model_identifier]):
        return jsonify({"error": "Missing player_id, season or model_id"}), 400

    try:
        player_metadata = next((data for name, data in player_index_main_data.items() if str(data.get("player_id")) == player_id_str), None)
        player_name = next((name for name, data in player_index_main_data.items() if str(data.get("player_id")) == player_id_str), "N/A")

        if not player_metadata:
            return jsonify({"error": f"Player metadata not found for ID {player_id_str}"}), 404

        position_group = trainer_get_general_position(player_metadata.get("position", "Unknown"))
        if position_group not in ["Attacker", "Midfielder", "Defender"]:
            return jsonify({"error": f"Prediction not supported for position group: {position_group}"}), 400

        dob = player_metadata.get("dob")
        if not dob: return jsonify({"error": "Player DOB not found"}), 400
        age = get_age_at_fixed_point_in_season(dob, season_to_predict_for)
        if age is None: return jsonify({"error": "Could not calculate age"}), 400

        is_custom = model_identifier != "default_v14"
        effective_model_id = "peak_potential_v2_15_16" if not is_custom else model_identifier
        base_path = "ml_models/ml_model_files_peak_potential" if not is_custom else "ml_models/custom_models"
        
        model_key = f"{base_path}/{effective_model_id}/{position_group.lower()}/potential_model_{position_group.lower()}_{effective_model_id}.joblib"
        scaler_key = f"{base_path}/{effective_model_id}/{position_group.lower()}/feature_scaler_{position_group.lower()}_{effective_model_id}.joblib"
        config_key = f"{base_path}/{effective_model_id}/{position_group.lower()}/model_config_{position_group.lower()}_{effective_model_id}.json"

        try:
            model_obj = s3_client.get_object(Bucket=R2_BUCKET_NAME, Key=model_key)['Body'].read()
            scaler_obj = s3_client.get_object(Bucket=R2_BUCKET_NAME, Key=scaler_key)['Body'].read()
            config_obj = s3_client.get_object(Bucket=R2_BUCKET_NAME, Key=config_key)['Body'].read()
            
            model = joblib.load(BytesIO(model_obj))
            scaler = joblib.load(BytesIO(scaler_obj))
            config = json.loads(config_obj.decode('utf-8'))
            expected_features = config.get("features_used_for_ml_model", [])
        except s3_client.exceptions.NoSuchKey as e:
            logger.error(f"A model file was not found in R2: {e.response['Error']['Key']}")
            return jsonify({"error": f"Model files for '{model_identifier}' are not available for this position."}), 404
        except Exception as e:
            logger.error(f"Failed to load model files from R2 for {model_identifier}. Error: {e}", exc_info=True)
            return jsonify({"error": f"Could not load model files for '{model_identifier}' from cloud storage."}), 500

        minutes_df_global = pd.read_csv(StringIO(s3_client.get_object(Bucket=R2_BUCKET_NAME, Key="data/player_season_minutes_with_names.csv")['Body'].read().decode('utf-8')))
        minutes_df_global['season_name_std'] = minutes_df_global['season_name'].str.replace('/', '_', regex=False)
        
        all_player_seasons = player_metadata.get("seasons", [])
        target_season_numeric = int(season_to_predict_for.split('_')[0])
        base_features_list = []

        for season in sorted(all_player_seasons):
            season_numeric = int(season.split('_')[0])
            if season_numeric > target_season_numeric: continue
            
            age_for_season = get_age_at_fixed_point_in_season(dob, season)
            if age_for_season is None or (age_for_season > 21 and season != season_to_predict_for): continue
                
            minutes_row = minutes_df_global[(minutes_df_global['player_id'].astype(str) == player_id_str) & (minutes_df_global['season_name_std'] == season)]
            num_90s = trainer_safe_division(minutes_row['total_minutes_played'].iloc[0], 90.0) if not minutes_row.empty else 0.0
            
            df_events = load_player_data(player_id_str, season)
            if df_events is None: df_events = pd.DataFrame()

            base_features = trainer_extract_base_features(df_events, age_for_season, season_numeric, num_90s)
            base_features['target_season_identifier'] = season
            base_features_list.append(base_features)

        if not base_features_list:
            return jsonify({"error": "Insufficient historical/current data for prediction."}), 404

        df_base_features = pd.DataFrame(base_features_list).fillna(0.0)
        current_season_row = df_base_features[df_base_features['target_season_identifier'] == season_to_predict_for]
        if current_season_row.empty:
            return jsonify({"error": f"Base features for target season {season_to_predict_for} not found."}), 404
        
        historical_df = df_base_features[df_base_features['season_numeric'] < target_season_numeric]
        
        ml_features = trainer_construct_ml_features_for_player_season(
            current_season_base_features_row=current_season_row.iloc[0],
            historical_base_features_df=historical_df,
            all_base_metric_names=trainer_get_feature_names()
        )
        
        features_df = pd.DataFrame([ml_features])
        aligned_df = features_df.reindex(columns=expected_features, fill_value=0.0)
        
        scaled_features = scaler.transform(aligned_df)
        prediction = model.predict(scaled_features)[0]
        final_score = min(200.0, max(0.0, float(prediction)))

        num_90s_target_season_row = minutes_df_global[(minutes_df_global['player_id'].astype(str) == player_id_str) & (minutes_df_global['season_name_std'] == season_to_predict_for)]
        num_90s_target = trainer_safe_division(num_90s_target_season_row['total_minutes_played'].iloc[0], 90.0) if not num_90s_target_season_row.empty else 0.0

        return jsonify({
            "player_id": player_id_str, "player_name": player_name,
            "season_predicted_from": season_to_predict_for,
            "age_at_season_start_of_year": age, "position_group": position_group,
            "predicted_potential_score": round(final_score, 2),
            "num_90s_played_in_season": round(num_90s_target, 2),
            "model_used": model_identifier,
        })

    except Exception as e:
        logger.error(f"Unexpected error in /scouting_predict: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred during prediction."}), 500

# --- Finalització de l'App Flask ---
